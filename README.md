...
- 截图 OCR 翻译：默认快捷键 `Control + Option + Space`，框选屏幕区域后使用 Apple Vision 本机 OCR。
- 沉浸式翻译浮窗：支持复制、重新翻译、固定、自动隐藏、收藏、查看历史和耗时展示。
...
- `Control + Option + Space`：框选屏幕区域，OCR 后翻译。
...
- OCR 目前会直接翻译识别结果，还缺少“识别文本预览 / 手动修正 / 重新框选”的完整确认流程。
...
- OCR 框选确认：框选后显示识别文本预览，支持编辑、重新框选、复制原文、确认翻译。
...
- `Sources/ImmersiveTranslator/OCRReader.swift`：使用 Apple Vision 做本机 OCR。
- `Sources/ImmersiveTranslator/TranslationClient.swift`：调用 OpenAI Chat Completions 兼容接口。
- `Sources/ImmersiveTranslator/TranslationPanel.swift`：翻译浮窗、复制、重试、收藏、历史入口。
...
正式分发需要 Apple Developer ID 证书和 notarytool 公证。
